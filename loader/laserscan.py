...
class SemLaserScan(LaserScan):
  ...
  def reset(self):
    ...
    self.sem_rd_label = np.zeros((0, 1), dtype=np.uint32)         # [m, 1]: label
    ...
    self.proj_sem_rd_label = np.zeros((self.proj_H, self.proj_W),
                                   dtype=np.int32)              # [H,W]  label

    # projection color with instance labels
    self.proj_inst_label = np.zeros((self.proj_H, self.proj_W),
                                    dtype=np.int32)              # [H,W]  label
    self.proj_inst_rd_label = np.zeros((self.proj_H, self.proj_W),
                                    dtype=np.int32)              # [H,W]  label
  ...
  def open_road_label(self, filename):
    """ Open raw scan and fill in attributes
    """
    # if all goes well, open label
    # label = np.load(filename)
    ...
    # only fill in attribute if the right size
    if label.shape[0] == self.points.shape[0]:
      self.sem_rd_label = label & 0xFFFF  # semantic label in lower half
      self.inst_rd_label = label >> 16    # instance id in upper half
    else:
      print("Points shape: ", self.points.shape)
      print("Label shape: ", label.shape)
      raise ValueError("Scan and Label don't contain same number of points")
    # print(np.unique(self.inst_rd_label))
    ...
    if self.project:
      ...
      # semantics
      self.proj_sem_rd_label[self.mask] = self.sem_rd_label[self.proj_idx[self.mask]]
      print(np.unique(self.proj_sem_rd_label))
      # instances
      self.proj_inst_rd_label[self.mask] = self.inst_rd_label[self.proj_idx[self.mask]]